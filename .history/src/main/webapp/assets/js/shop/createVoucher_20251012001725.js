...
function submitVoucher() {
  ...
  formData.append("voucherCode", document.getElementById("voucherCode").value);
  formData.append(
    "voucherDescription",
  ...
  formData.append("maxDiscount", document.getElementById("maxDiscount").value);
  formData.append(
    "minOrderValue",
    document.getElementById("minOrderValue").value
  );
  formData.append("usageLimit", document.getElementById("usageLimit").value);
  formData.append("startDate", document.getElementById("startDate").value);
  formData.append("endDate", document.getElementById("endDate").value);

  console.log("Creating voucher with data:", formData);

  fetch("/shop/voucher?action=create", {
    method: "POST",
    body: formData,
  })
    .then((response) => {
    ...
    .catch((error) => {
      ...
      submitButton.classList.remove("loading");
      submitButton.innerHTML =
        '<i class="bi bi-exclamation-triangle me-2"></i>Thử lại';

      showErrorMessage("Tạo voucher thất bại, vui lòng thử lại!");
    });
}
