...
document.addEventListener("DOMContentLoaded", function () {
  ...
  fileInput.addEventListener("change", function (event) {
    ...
    // Giữ đúng thứ tự upload bằng cách nối cuối danh sách
  ...
  function renderPreview() {
    ...
    // Load lần lượt để giữ đúng thứ tự
    selectedFiles.forEach((file, index) => {
      ...
      reader.onload = function (e) {
        ...
        // Nếu là ảnh đầu tiên => thêm nhãn “Ảnh đại diện”
        const isMain = index === 0;
        const mainBadge = isMain
          ? `<span class="badge bg-primary position-absolute top-0 start-0 m-1"
                    style="z-index: 2; font-size: 0.75rem;">Ảnh đại diện</span>`
          : "";
