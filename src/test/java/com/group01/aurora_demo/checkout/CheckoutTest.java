...
@Test
void testSuccessfulCheckout_WithNewAddress_AndVoucher() throws
...
WebElement chatBox =
wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("chatbot-box")));
Thread.sleep(1500);
...
.findElement(By.cssSelector("#chatbot-messages .message-bot:last-child
.bubble"));
...
System.out.println("Không thể trích xuất từ khóa, dùng từ khóa dự phòng
'Sherlock Holmes'.");
...
Thread.sleep(1000);
wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector("div.product
a"))).click();
...
Assertions.assertTrue(isSuccess, "Thêm vào giỏ hàng thất bại. Thông báo lỗi:
" + toastMessage);
...
System.out.println("BƯỚC 8A: Không tìm thấy địa chỉ. Bắt đầu điền modal 'Thêm
địa chỉ'...");
...
new
Select(addressModal.findElement(By.id("addProvince"))).selectByVisibleText(ADDR_PROVINCE);
Thread.sleep(1000);

// Xử lý Dropdown Quận/Huyện
new
Select(addressModal.findElement(By.id("addDistrict"))).selectByVisibleText(ADDR_DISTRICT);
Thread.sleep(1000);

// Xử lý Dropdown Phường/Xã
new
Select(addressModal.findElement(By.id("addWard"))).selectByVisibleText(ADDR_WARD);
Thread.sleep(1000);
...
addressModal.findElement(By.cssSelector("#form-create-address
...
System.out.println("BƯỚC 9.1: Đã chuyển hướng đến VNPAY - Trang 1 (Chọn
phương thức).");
...
System.out.println("BƯỚC 10.1: Đã chuyển đến VNPAY - Trang 2 (Điền thông tin
thẻ).");
...
Assertions.assertTrue(isSuccess, "Đặt hàng thất bại. Thông báo lỗi: " +
toastMessage);
