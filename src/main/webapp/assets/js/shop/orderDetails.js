...
function updateCustomerInfo(customer) {
  const customerInfoRows = document.querySelectorAll(
...
function updatePaymentSummary(payment) {
  ...
  if (summaryRows.length >= 4) {
    summaryRows[0].querySelector(
      "span:last-child"
    ).textContent = `${payment.subtotal.toLocaleString()} VNĐ`;
    summaryRows[1].querySelector(
      "span:last-child"
    ).textContent = `${payment.shipping.toLocaleString()} VNĐ`;
    summaryRows[2].querySelector(
      "span:last-child"
    ).textContent = `${payment.discount.toLocaleString()} VNĐ`;
    summaryRows[3].querySelector(
      "strong:last-child"
    ).textContent = `${payment.total.toLocaleString()} VNĐ`;
  }
}
