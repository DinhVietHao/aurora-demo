...
<body class="sb-nav-fixed">
        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid px-4">
                    ...
                    <!-- System Revenue Settings Card -->
                    <div class="row mt-4">
                        <div class="col-12">
                            <div class="card border-primary">
                                <div class="card-header bg-primary text-white">
                                    <h5 class="mb-0"><i class="bi bi-cash-coin me-2"></i>Cài đặt Thu nhập Hệ thống</h5>
                                </div>
                                <div class="card-body">
                                    <div class="alert alert-info mb-3">
                                        <i class="bi bi-info-circle me-2"></i>
                                        <strong>Lưu ý:</strong> Thu nhập hệ thống = Thuế VAT theo sách + Phí hoa hồng theo đơn hàng
                                    </div>
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="card bg-light">
                                                <div class="card-body">
                                                    <h6 class="card-title">
                                                        <i class="bi bi-receipt me-2"></i>Phí hệ thống theo đơn hàng
                                                    </h6>
                                                    <c:set var="systemFee" value="5.0" />
                                                    <c:forEach var="setting" items="${settingList}">
                                                        <c:if test="${setting.settingKey == 'SYSTEM_FEE_PER_ORDER'}">
                                                            <c:set var="systemFee" value="${setting.settingValue}" />
                                                            <c:set var="systemFeeId" value="${setting.settingId}" />
                                                        </c:if>
                                                    </c:forEach>
                                                    <div class="d-flex align-items-center justify-content-between mt-3">
                                                        <div>
                                                            <h2 class="mb-0 text-primary">${systemFee}%</h2>
                                                            <small class="text-muted">Phần trăm hoa hồng trên mỗi đơn hàng</small>
                                                        </div>
                                                        <button class="btn btn-outline-primary btn-sm" 
                                                                onclick="editSystemFee('${systemFeeId}', '${systemFee}')">
                                        ...
                                        <div class="col-md-6">
                                            <div class="card bg-light">
                                                <div class="card-body">
                                                    <h6 class="card-title">
                                                        <i class="bi bi-percent me-2"></i>Thuế VAT theo sách
                                                    </h6>
                                                    <div class="mt-3">
                                                        <p class="mb-1"><i class="bi bi-arrow-right me-2"></i>Quản lý tại phần <strong>Quản lý VAT</strong></p>
                                                        <a href="${pageContext.request.contextPath}/admin/vat-management" 
                                                           class="btn btn-outline-secondary btn-sm mt-2">
                                                            <i class="bi bi-box-arrow-up-right me-1"></i>Đi đến Quản lý VAT
                                                        </a>
                                                    </div>
                                                </div>
                                            </div>
    ...
    <script>
        // Edit System Fee with validation
        function editSystemFee(settingId, currentValue) {
            // If setting doesn't exist yet, we need to create it first
            if (!settingId || settingId === '') {
                // Show add modal with pre-filled values
                document.getElementById('settingKey').value = 'SYSTEM_FEE_PER_ORDER';
                document.getElementById('settingValue').value = '5.0';
                document.getElementById('description').value = 'Phí hệ thống tính theo phần trăm (%) cho mỗi đơn hàng. Giá trị từ 0-100.';
                
                // Add validation for percentage
                const form = document.querySelector('#addSettingModal form');
                form.onsubmit = function(e) {
                    const value = parseFloat(document.getElementById('settingValue').value);
                    if (isNaN(value) || value < 0 || value > 100) {
                        e.preventDefault();
                        alert('Phí hệ thống phải là số từ 0 đến 100 (%)');
                        return false;
                    }
                };
                
                const addModal = new bootstrap.Modal(document.getElementById('addSettingModal'));
                addModal.show();
            } else {
                // Edit existing setting
                document.getElementById('editSettingId').value = settingId;
                document.getElementById('editSettingKey').value = 'SYSTEM_FEE_PER_ORDER';
                document.getElementById('editSettingKey').readOnly = true; // Don't allow changing key
                document.getElementById('editSettingValue').value = currentValue;
                document.getElementById('editDescription').value = 'Phí hệ thống tính theo phần trăm (%) cho mỗi đơn hàng. Giá trị từ 0-100.';
                
                // Add validation for percentage
                const form = document.querySelector('#editSettingModal form');
                form.onsubmit = function(e) {
        ...
        function editSetting(settingId, settingKey, settingValue, description) {
            ...
            document.getElementById('editSettingKey').readOnly = false; // Allow changing for regular settings
            document.getElementById('editSettingValue').value = settingValue || '';
            document.getElementById('editDescription').value = description || '';
            
            // Remove special validation for regular settings
            const form = document.querySelector('#editSettingModal form');
            form.onsubmit = null;
