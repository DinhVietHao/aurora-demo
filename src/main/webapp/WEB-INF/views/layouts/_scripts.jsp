<%@page contentType="text/html" pageEncoding="UTF-8" %>
    <%@ taglib prefix="c" uri="jakarta.tags.core" %>
        ...
        <c:url var="googleAuthUrl" value="https://accounts.google.com/o/oauth2/v2/auth">
            ...
            <c:param name="prompt" value="consent" />
        </c:url>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                const shopStatusBtn = document.getElementById('shopStatusBtn');
                if (shopStatusBtn) {
                    shopStatusBtn.addEventListener('click', function () {
                        fetch('${ctx}/shop?action=check-status', {
                            method: 'GET',
                        })
                            .then(res => res.json())
                            .then(data => {
                                if (data.status === 'ACTIVE') {
                                    ...
                                    // Optional: Redirect đến đăng ký sau toast
                            ...
                            .catch(error => {
                                ...
                                toast({
                                    title: "Lỗi hệ thống",
                                    message: "Không thể kiểm tra trạng thái shop.",
                                    type: "error",
                                    duration: 5000,
                                });
                            });
