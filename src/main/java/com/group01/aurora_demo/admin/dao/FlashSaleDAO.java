...
public class FlashSaleDAO {
    ...
    /**
     * Kiểm tra xem có Flash Sale ACTIVE nào đang chạy trong khoảng thời gian này không
     * 
     * @param startAt Thời gian bắt đầu
     * @param endAt Thời gian kết thúc
     * @param excludeId ID của Flash Sale cần loại trừ (dùng khi update), truyền -1 nếu tạo mới
     * @return true nếu có conflict, false nếu không có conflict
     */
    public boolean hasActiveFlashSaleOverlap(java.sql.Timestamp startAt, java.sql.Timestamp endAt, long excludeId) throws SQLException {
        // Check if there's any ACTIVE flash sale that overlaps with the given time range
        String sql = "SELECT COUNT(*) FROM FlashSales " +
                     "WHERE Status = 'ACTIVE' " +
                     "AND FlashSaleID != ? " +
                     "AND NOT (EndAt <= ? OR StartAt >= ?)"; // Overlap condition
        
        try (Connection cn = DataSourceProvider.get().getConnection();
             PreparedStatement ps = cn.prepareStatement(sql)) {
            ps.setLong(1, excludeId);
            ps.setTimestamp(2, startAt); // Other flash sale ends before this one starts
            ps.setTimestamp(3, endAt);   // Other flash sale starts after this one ends
            
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                return rs.getInt(1) > 0; // Return true if there's any overlapping ACTIVE flash sale
            }
            return false;
        }
    }
    ...
    public int updateApprovalStatus(long flashSaleItemId, String approvalStatus) throws SQLException {
        // When rejecting, restore the product stock from FsStock
        if ("REJECTED".equals(approvalStatus)) {
            restoreProductStockFromFlashSaleItem(flashSaleItemId);
        }
        ...
        if ("APPROVED".equals(approvalStatus)) {
            ...
            if (productId > 0) {
                // First, get all other FlashSaleItemIDs for this product to restore their stock
                String getOtherItemsSql = "SELECT FlashSaleItemID FROM FlashSaleItems WHERE ProductID = ? AND FlashSaleItemID != ? AND ApprovalStatus != 'REJECTED'";
                try (Connection cn = DataSourceProvider.get().getConnection();
                     PreparedStatement ps = cn.prepareStatement(getOtherItemsSql)) {
                    ps.setLong(1, productId);
                    ps.setLong(2, flashSaleItemId);
                    ResultSet rs = ps.executeQuery();
                    
                    // Restore stock for each item that will be rejected
                    while (rs.next()) {
                        long otherItemId = rs.getLong("FlashSaleItemID");
                        restoreProductStockFromFlashSaleItem(otherItemId);
                    }
                }
                
                // Then mark any other items for this product as REJECTED to prevent duplicates
    ...
    /**
     * Restore product stock when Flash Sale item is rejected
     * Add FsStock back to Products.Quantity
     * Also update Status from OUT_OF_STOCK to ACTIVE if needed
     */
    private void restoreProductStockFromFlashSaleItem(long flashSaleItemId) throws SQLException {
        String getStockSql = "SELECT ProductID, FsStock FROM FlashSaleItems WHERE FlashSaleItemID = ?";
        String updateProductSql = "UPDATE Products SET Quantity = Quantity + ?, " +
                                  "Status = CASE " +
                                  "  WHEN Status = 'OUT_OF_STOCK' AND (Quantity + ?) > 0 THEN 'ACTIVE' " +
                                  "  ELSE Status " +
                                  "END " +
                                  "WHERE ProductID = ?";
        ...
        try (Connection cn = DataSourceProvider.get().getConnection()) {
            ...
            // Get the ProductID and FsStock from FlashSaleItem
            try (PreparedStatement ps = cn.prepareStatement(getStockSql)) {
            ...
            // Restore the stock back to Products table and update status if needed
            if (productId > 0 && fsStock > 0) {
                try (PreparedStatement ps = cn.prepareStatement(updateProductSql)) {
                    ps.setInt(1, fsStock);  // For Quantity = Quantity + ?
                    ps.setInt(2, fsStock);  // For CASE condition (Quantity + ?)
