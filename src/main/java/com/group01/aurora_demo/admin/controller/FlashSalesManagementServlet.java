...
public class FlashSalesManagementServlet extends HttpServlet {
    ...
    private void handleCreate(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        if ("ACTIVE".equalsIgnoreCase(status) && startAt != null && endAt != null) {
            try {
                boolean hasOverlap = dao.hasActiveFlashSaleOverlap(startAt, endAt, -1);
                if (hasOverlap) {
                    errors.append("Đã có Flash Sale ACTIVE khác đang hoạt động trong khoảng thời gian này. Chỉ được phép 1 Flash Sale ACTIVE cùng lúc! ");
                }
            } catch (SQLException e) {
                errors.append("Error checking flash sale overlap: " + e.getMessage() + " ");
            }
        }
    ...
    private void handleUpdate(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        // Check for overlapping ACTIVE flash sales (excluding current one)
        if ("ACTIVE".equalsIgnoreCase(status) && startAt != null && endAt != null) {
            try {
                boolean hasOverlap = dao.hasActiveFlashSaleOverlap(startAt, endAt, id);
                if (hasOverlap) {
                    errors.append("Đã có Flash Sale ACTIVE khác đang hoạt động trong khoảng thời gian này. Chỉ được phép 1 Flash Sale ACTIVE cùng lúc! ");
                }
            } catch (SQLException e) {
